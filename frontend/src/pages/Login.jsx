...
const Login = () => {
    ...
    const onSubmitForm=async (event)=>{
        ...
        try{
        if(state==="sign up"){
            const {data}=await axios.post("https://arhospitalapp-1.onrender.com/api/user/register",{name,email,password});
            if(data.success){
             ...
             const {data}=await axios.post("https://arhospitalapp-1.onrender.com/api/user/login",{email,password});
            if(data.success){
                localStorage.setItem("token",data.token);
                setAToken(data.token);
                toast.success()
